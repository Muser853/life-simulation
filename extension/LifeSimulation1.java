...
public class LifeSimulation1 extends Application {
    ...
    private void startRecording() {
        ...
        isRecording.compareAndSet(false, true); // 使用 compareAndSet 确保线程安全
        frameCount = 0;
    ...
    private void stopRecording() {
        ...
        isRecording.compareAndSet(true, false); // 使用 compareAndSet 确保线程安全

        // using FFmpeg to synthesize video
        try {
            String outputVideo = "output_video_" + System.currentTimeMillis() + ".mp4";
            ProcessBuilder processBuilder = new ProcessBuilder(
                "ffmpeg",
                "-framerate", "30", // frane rate
            ...
            process.waitFor(); // wait for FFmpeg to end
    ...
    private void updateChart(String key, double chance, double[] data) {
        ...
        List<XYChart.Data<Number, Number>> dataPoints = mainSeries.getData();
        dataPoints.add(new XYChart.Data<>(chance, avg));
        dataPoints.sort(Comparator.comparingDouble(d -> d.getXValue().doubleValue()));
        ...
        upperSeries.getNode().setStyle("-fx-stroke: red; -fx-stroke-dash-array: 0.5 0.5;");
        lowerSeries.getNode().setStyle("-fx-stroke: green; -fx-stroke-dash-array: 0.5 0.5;");
    ...
    // Initialize components
        ...
        simulationCanvas = new Canvas(max * CELL_SIZE, max * CELL_SIZE);
            ...
            // generate 3d contents
                ...
                for (int m = MIN_SIZE; m <= max; m++) {
                    for (int n = MIN_SIZE; n <= m; n++) {
                        ...
                        IntStream.rangeClosed(0, 20).parallel().forEach(i -> { // 使用并行流
                            double chance = i * 0.05;
                            ...
                            Platform.runLater(() -> {
                                updateChart(key, finalChance, new double[]{averageLivingCells, 0});
                            });
                        });
                    }
                }
