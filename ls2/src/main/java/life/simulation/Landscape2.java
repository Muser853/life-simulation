...
public class Landscape2 {
    ...
    private final int rows, columns, depth, dimensions, lower, low, up, upper;
    private final double initialChance;
    private Map<List<Integer>, Cell> highDimLandscape;

    public Landscape2(int rows, int columns, int depth, double initialChance, int lower, int low, int up, int upper, GridType gridType, int dimensions) {
        this.rows = rows;
    ...
    public Landscape2(int rows, int columns, int depth, double initialChance, int lower, int low, int up, int upper) {
    ...
    public ArrayList<Cell> getNeighbors(int row, int col, int z) {
        return switch (gridType) {
            case TRIANGLE -> getTriangleNeighbors(row, col, z);
            case HEXAGON -> getHexagonNeighbors(row, col, z);
            default -> getSquareNeighbors(row, col, z);
        };
    }
